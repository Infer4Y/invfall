...
public class CommonThread extends Thread{
    ...
    public void run() {
        while (!Thread.currentThread().isInterrupted() && DisplayReference.running) {
            ...
            DisplayReference.delta_dime += (DisplayReference.current_time - DisplayReference.last_loop_time) / DisplayReference.OPTIMAL_TIME;
            DisplayReference.last_loop_time = DisplayReference.current_time;

            while (DisplayReference.delta_dime >= 1) {
                ClientUtils.update();
                DisplayReference.delta_dime--;
            }
        }
    }
}
