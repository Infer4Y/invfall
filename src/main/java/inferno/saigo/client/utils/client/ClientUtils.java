...
public class ClientUtils {
    public static void update(double delta_dime) {
        if (!(DisplayReference.renderer.camera.getX() >  0)) {
            DisplayReference.renderer.camera.update((1f / (float) ((DisplayReference.OPTIMAL_TICKS/delta_dime))) * 5, 0);
        } else {
        ...
        if (!(DisplayReference.renderer.camera.getY() >  0)) {
            DisplayReference.renderer.camera.update(0, (1f / (float) ((DisplayReference.OPTIMAL_TICKS/delta_dime)) * 5));
        } else {
